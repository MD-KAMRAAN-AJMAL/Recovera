...
async function check() {
  try {
    ...
    const recentAudits = await prisma.safetyAuditLog.findMany({
      take: 5,
      orderBy: { createdAt: 'desc' }
    });
    ...
    const pendingQueue = await prisma.detectionQueue.count({ where: { status: 'pending' } });
    const processingEvents = await prisma.incidentEvent.count({ where: { processingStatus: 'pending' } });
